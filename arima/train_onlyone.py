...
def auto_wrong_test():
    ...
    model = pmdarima.arima.auto_arima(df,start_p=0,start_q=0,alpha=0.01, trace=False,stepwise=False,suppress_warnings=True,error_action='ignore')
    pqd = model.summary().tables[0].data[1][1]
    print(pqd)
...
def save_auto():
    ...
    for idx,filename in enumerate(files):
        ...
        dftest = np.array(data[lens:lens+10]['cpulist'])
        df_mem = np.array(data[:lens]['memlist'])
        model_cpu = pmdarima.arima.auto_arima(df,start_p=0,start_q=0,alpha=0.01, trace=False,stepwise=False,suppress_warnings=True,error_action='ignore')
        #print('i=',i,'mem')
        model_mem =  pmdarima.arima.auto_arima(df_mem,start_p=0,start_q=0,alpha=0.01, trace=False,stepwise=False,suppress_warnings=True,error_action='ignore')
...
def demo_auto(filename):
    ...
    model_auto = pmdarima.arima.auto_arima(df,start_p=0,start_q=0,alpha=0.01, trace=True,stepwise=False,suppress_warnings=True,error_action='ignore')
...
def test(datafile,modelfile):
    ...
    model_auto = pmdarima.arima.AutoARIMA(start_p=0,start_q=0, max_d = 3,trace=True,stepwise=False,suppress_warnings=True,error_action='ignore').fit(df)
    model_auto_arima = pmdarima.arima.auto_arima(df,start_p=0,start_q=0,alpha=0.01, trace=True,stepwise=False,suppress_warnings=True,error_action='ignore')
