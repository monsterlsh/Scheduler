...
def using_model(cluster:Cluster,W,end,forecast_mem,forecast_cpu):
    ...
    for inc_id,instance in cluster.instances.items():
        ...
        # if (next_cpu > 0.01).all() and (actual > 0.01).all():
            
        #     try:
        #         forecast,actual = reduce0(next_cpu,actual)
        #     except:
        #         print(next_cpu.tolist(),actual.tolist())
        #         assert 1==0
        #     newmape = 0
        #     if actual.shape[0] != 0:
        ...
        tic = ends-start
        if flag and tic >0.1:
            print(f'\t\t {iters} consuming arima {tic}s cpu_wrong={cpu_wrong},mem_wrong={mem_wrong}')
    print(f'\t\tmape sum = {sums} avg = {sum/iters}')
    after =  time()
    print(f'\t\tthe consuming in arima is {(after-startPredict)}s avg mape = {sums/iters}  and  end is { end} ')
    #assert end <20  
