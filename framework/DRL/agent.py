...
class Agent(object):
    def __init__(self, name, policynet, gamma, reward_to_go, nn_baseline, normalize_advantages, model_save_path=None,
                 summary_path=None):
        super().__init__()
        ...
        self.checkpoint = tf.train.Checkpoint(policynet=self.policynet)
    ...
    def update_parameters(self, all_observations, all_actions, all_advantages):
        ...
        for observations, actions, advantages in zip(all_observations, all_actions, all_advantages):
            ...
            for observation, action, advantage in zip(observations, actions, advantages):
                ...
                grads = t.gradient(loss_value, self.policynet.variables)
    ...
    def optimize(self, grads_by_trajectory):
        ...
        assert len(average_grads) == len(self.policynet.variables)
        for average_grad, variable in zip(average_grads, self.policynet.variables):
            assert average_grad.shape == variable.shape

        self.optimizer.apply_gradients(zip(average_grads, self.policynet.variables), self.global_step)
